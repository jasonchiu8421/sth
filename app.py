    ...
    <body>
        ...
        <script>
            ...
            function endDrag(e) {{
                ...
                // If movement is mostly vertical or very small → treat as click, not swipe
                if (Math.abs(diffY) > 30 || absDiffX < 8) {{   // small movement = tap
                    toggleExpand();
                    return;
                }}
            ...
            function toggleExpand() {{
                ...
                // Only expand if the card is roughly in center (not during swipe)
                const currentTransform = parseFloat(visibleCard.style.transform.replace('translateX(', '').replace('px)', '')) || 0;
                if (Math.abs(currentTransform) < 30) {{
